...
(begin-for-test
  (local
    ((define PARTICLE1 (make-particle 75 50 0 0))
     (define PARTICLE2 (make-particle 35 45 -5 5))
     (define PARTICLE3 (make-particle 30 50 -5 5))
     (define PARTICLE4 (make-particle 0 45 -5 5))
     (define PARTICLE5 (make-particle 150 45 -5 5))
     (define CONTROLLER1 (new PositionController%
                              [model MODEL1]
                              [center-x 300]
                              [center-y 250])))
    
    (send MODEL1 register CONTROLLER1)
    (check-equal? (send (first (send MODEL1 for-test:controllers))
                        add-to-scene
                        EMPTY-CANVAS)
                  TEST-SCENE-4
                  "controller should be painted on the world")
    
    (check-equal? (send MODEL1 for-test:particle)
                  PARTICLE1
                  "should be at the default behaviour")
    
    (send MODEL1 execute-command (make-set-position 35 45))
    (send MODEL1 execute-command (make-incr-velocity -5 5))
    
    (check-equal? (send MODEL1 for-test:particle)
                  PARTICLE2
                  "should change position and velocity to desired values")
    
    (send MODEL1 after-tick)
    
    (check-equal? (send MODEL1 for-test:particle)
                  PARTICLE3
                  "should move to place at after-tick")
    
    (send MODEL1 execute-command (make-set-paused true))
    (send MODEL1 after-tick)
    ...
    (check-equal? (send MODEL1 for-test:paused?)
                  true
                  "should set the paused? to true")
    
    (send MODEL1 execute-command (make-set-position -35 45))
    (check-equal? (send MODEL1 for-test:particle)
                  PARTICLE4
                  "x must be set to minimum value")

    (send MODEL1 execute-command (make-set-position 155 45))
    (check-equal? (send MODEL1 for-test:particle)
                  PARTICLE5
                  "x must be set to maximum value")
    ))