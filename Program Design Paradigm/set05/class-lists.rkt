...
(define STUD0 (make-slip FELLEISEN-COLOR  "Dishant" "Kapadiya"))
(define STUD1 (make-slip FELLEISEN-COLOR  "Paulomi" "Mahidharia"))
(define STUD2 (make-slip FELLEISEN-COLOR  "Sanil" "Jain"))
(define STUD3 (make-slip FELLEISEN-COLOR  "Sudeep" "Kulkarni"))
(define STUD4 (make-slip FELLEISEN-COLOR  "Sushant" "Mimani"))
...
(define STUD9 (make-slip FELLEISEN-COLOR  "Kapadiya" "Dishant"))
...
(define (felleisen-roster los)
  (remove-dup (get-list-for-prof los FELLEISEN-COLOR)))
...
(define (shivers-roster los)
  (remove-dup (get-list-for-prof los SHIVERS-COLOR)))
...
;;; get-list-for-prof: ListOfSlip String -> ListOfSlip
;;; GIVEN: a ListOfSlip and a color of slip
;;; RETURNS: a list which has all skips of specified color
;;; EXAMPLES: (get-list-for-prof (list STUD0 STUD1 STUD7 STUD8 STUD0) FELLEISEN-COLOR)
;;;           = (list STUD0 STUD1 STUD0)
;;; DESIGN STRATEGY: use HOF filter on los
(define (get-list-for-prof los color)
  (filter
   ;;; Slip -> Boolean
   (lambda (x) (string=? (slip-color x) color))
   los))

;;; TESTS:
(begin-for-test
  (check-equal? (get-list-for-prof (list STUD0 STUD1 STUD7 STUD8 STUD0) FELLEISEN-COLOR)
                (list STUD0 STUD1 STUD0)
                "extracted the slips of prof. felleisen")
  (check-equal? (get-list-for-prof (list STUD0 STUD1 STUD7 STUD8 STUD0) SHIVERS-COLOR)
                (list STUD7 STUD8)
                "extracted the slips of prof. shivers"))

;;; remove-dup: ListOfSlip -> ListOfSlip
...
(define (remove-dup los)
  (foldr
   ...
   (lambda (x r) (if (is-it-distinct? x r) (cons x r) r))
   empty los))
...
;;; DESIGN STRATEGY: use HOF andmap on s and los
(define (is-it-distinct? s los)
  (andmap
   ;;; Slip -> Boolean
   (lambda (x) (not (is-it-dup? s x)))
   los))
