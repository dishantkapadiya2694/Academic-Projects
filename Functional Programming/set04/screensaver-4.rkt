...
(define DOT-IMAGE (circle 1 "solid" "black"))
...
;;;                        NonNegInt NonNegInt Integer Boolean)
;;; Interpretation:
...
;;; pen-down? true if the dots are getting printed
;;;
;;; templete:
;;; rect-fn: Rect -> ??
#|
...
;;; a WorldState is a (make-world rect rect Boolean)
;;; Interpretation:
...
;;; EXAMPLES: 
;;; DESIGN STRATEGY: use templete for ListOfRectangle on lor
(define (update-dots lor lod)
  (cond
    [(empty? lor) lod]
    [else
     (update-dots (rest lor) (add-dot (first lor) lod))]))
...
;;; toggle-dots: Rectangle Boolean -> Rectangle
;;; GIVEN: a Rectangle and a value for pen-down? field in Rectangle
;;; RETURNS: a Rectangle that has its value updated
;;; EXAMPLE: (toggle-dots INIT-RECT true) = (make-rect 200 150 0 0 #false 0 0 0 0 #true)
;;;          (toggle-dots INIT-RECT false) = (make-rect 200 150 0 0 #false 0 0 0 0 #false)
;;; DESIGN STRATEGY: use templete of Rectangle on r
(define (toggle-dots r val)
...
;;; TESTS:
(begin-for-test
  (check-equal? (toggle-dots INIT-RECT true)
                (make-rect 200 150 0 0 #false 0 0 0 0 #true)
                "sets pen-down? to true")
  (check-equal? (toggle-dots INIT-RECT false)
...
;;; EXAMPLE:
;;; DESIGN STRATEGY: use templete of ListOfDot on lod
(define (place-dots lod)
  (cond
    [(empty? lod) EMPTY-CANVAS]
    [else
     (place-image DOT-IMAGE (dot-x (first lod)) (dot-y (first lod))
                  (place-dots (rest lod)))]))
